...
export function AISidebar({ isOpen, onClose, questionContext }: AISidebarProps) {
    ...
    return (
        <div className={cn(
            "fixed inset-y-0 right-0 w-full md:w-[450px] bg-card border-l border-border shadow-2xl transform transition-transform duration-500 ease-in-out z-[60] flex flex-col",
            isOpen ? "translate-x-0" : "translate-x-full"
        )}>
            {/* Header */}
            <div className="h-20 border-b border-border flex items-center justify-between px-6 bg-secondary/30 backdrop-blur-md">
